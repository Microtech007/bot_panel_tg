...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Microtech007/bot_panel_tg/main/access > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Microtech007/bot_panel_tg/main/access | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Microtech007/bot_panel_tg/main/access | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
dircreate
        ...
        echo -e "MTS Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @MTS] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@MTS"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: MTS-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="MTS-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/Microtech007/bot_panel_tg/main/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
			[[ "$(grep -wc "MTS_bot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS MTS_bot bbt' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS MTS_bot bbt >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "MTS_bot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 2
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "MTS_bot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "MTS_bot" /etc/rc.local) != '0' ]] && {
                sed -i '/MTS_bot/d' /etc/rc.local
            }
